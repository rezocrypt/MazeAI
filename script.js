...
const max_distance = Math.sqrt(
        constant_height * constant_height + constant_width * constant_width,
);

function one_step_vertical(start_x, start_y, stop_x, stop_y, row) {
        let range_x = 0;
        let range_y = 0;
        let random_hole = Math.floor(
                Math.random() * (stop_y - range_y - (start_y + range_y)) +
                (start_y + range_y),
        );
        ...
        for (var i = start_y; i < stop_y; i++) labyrinth[i][row][1] = 1;
        labyrinth[random_hole][row][1] = 0;
...
function one_step_horizontal(start_x, start_y, stop_x, stop_y, column) {
        ...
        let range_y = 0;
        let random_hole = Math.floor(
                Math.random() * (stop_x - range_x - (start_x + range_x)) +
                (start_x + range_x),
        );
        ctx.strokeStyle = '#000000';
        ctx.beginPath();
        ...
        for (var i = 0; i < 100; i++) ctx.stroke();
        for (var i = start_x; i < stop_x; i++) labyrinth[column][i][0] = 1;
        labyrinth[column][random_hole][0] = 0;
        ctx.strokeStyle = '#ffffff';
        ctx.beginPath();
...
function create_labyrinth_matrix() {
        for (let i = 0; i < constant_height; i++) {
                ...
                labyrinth[i] = height_element;
        }
        labyrinth[player_y][player_x][2] = 1;
}
...
function recursive_step(start_x, start_y, stop_x, stop_y) {
        ...
        let index = Math.floor(
                Math.random() * (stop_y - range - (start_y + range)) +
                (start_y + range),
        );
        if (width >= height) {
                ori = 'v';
                index = Math.floor(
                        Math.random() * (stop_x - range - (start_x + range)) +
                        (start_x + range),
                );
        }
        if (width <= 2 || height <= 2) {
...
function move_player(coordinates) {
        if (labyrinth[coordinates[0]][coordinates[1]][2] == 1) {
        ...
        update_steps();
        update_player();
        labyrinth[player_y][player_x][2] = labyrinth[player_y][player_x][2] + 1;
}
...
function update_weight_info() {
        ...
        a.textContent = distance_weight.toFixed(2).toString();
        b.textContent = step_count_weight.toFixed(2).toString();
        c.textContent = valid_steps_weight.toFixed(2).toString();
        d.textContent = points.toFixed(2).toString();
}

function find_distance_to_exit(coord) {
        let y = coord[0];
        let x = coord[1];
        let old_value = Math.sqrt(
                (constant_height - y) * (constant_height - y) +
                (constant_width - x) * (constant_width - x),
        );
...
function step_count_range_changer(coord) {
        let new_value =
                ((labyrinth[coord[0]][coord[1]][2] - 0) / (2 - 0)) * (10 - 0) +
                0;
        return new_value;
}

// Function for calculating points for coordinage
...
// Function for finding neighbours rate
function find_valid_neighbours_steps_rate(coord) {
        ...
        for (let i = 0; i < all_moves.length; i++) {
                ...
                if (
                        all_moves[i][0] < coord[0] &&
                        labyrinth[coord[0]][coord[1]][0] == 1
                ) {
                        continue;
                }
                if (
                        all_moves[i][1] < coord[1] &&
                        labyrinth[coord[0]][coord[1]][1] == 1
                ) {
                        continue;
                }
                if (
                        all_moves[i][0] > coord[0] &&
                        labyrinth[coord[0] + 1][coord[1]][0] == 1
                ) {
                        continue;
                }
                if (
                        all_moves[i][1] > coord[1] &&
                        labyrinth[coord[0]][coord[1] + 1][1] == 1
                ) {
                        continue;
                }
                if (labyrinth[all_moves[i][0]][all_moves[i][1]][2] >= 2) {
                        continue;
                }
                valid_moves.push(all_moves[i]);
        }
        for (let i = 0; i < valid_moves.length; i++) {
                old_value += find_valid_steps_rate([
                        valid_moves[i][0],
                        valid_moves[i][1],
                ]);
        }
...
// Functin for finding rate of step
...
function find_best_move() {
        ...
        for (let i = 0; i < all_moves.length; i++) {
                ...
                if (
                        all_moves[i][0] < player_y &&
                        labyrinth[player_y][player_x][0] == 1
                ) {
                        continue;
                }
                if (
                        all_moves[i][1] < player_x &&
                        labyrinth[player_y][player_x][1] == 1
                ) {
                        continue;
                }
                if (
                        all_moves[i][0] > player_y &&
                        labyrinth[player_y + 1][player_x][0] == 1
                ) {
                        continue;
                }
                if (
                        all_moves[i][1] > player_x &&
                        labyrinth[player_y][player_x + 1][1] == 1
                ) {
                        continue;
                }
                if (labyrinth[all_moves[i][0]][all_moves[i][1]][2] >= 2) {
        ...
        for (let i = 0; i < valid_moves.length; i++) {
                moves_scores[i] = calculate_points(
                        valid_moves[i],
                        distance_weight_test,
                        step_count_weight_test,
                        valid_steps_weight_test,
                );
        }
        let best_move =
                valid_moves[
                moves_scores.indexOf(Math.max.apply(0, moves_scores))
                ];
        return best_move;
}

function reset_labyrinth() {
        points = 10;
        clear_player();
        player_x = 0;
        player_y = 0;
